...
//  \brief Initial data reader for black hole - neutron star (BHNS) binary data
//         produced by Elliptica.
//
//  Usage
//  -----
//  Copy this file to src/pgen/ and configure with
//    cmake .. -DPROBLEM=elliptica_bhns
//
//  The matching input file is elliptica_bhns.athinput (in this same directory).
//
//  Key differences from elliptica_bns.cpp
//  ----------------------------------------
//  * Only one neutron star → EoS table parameter is "NS_EoS_table_path"
//    (not "NS1_EoS_table_path" / "NS2_EoS_table_path").
//  * Density and pressure returned by Elliptica are physically meaningful only
//    inside the neutron star; cells inside or near the BH puncture receive
//    atmosphere (floor) values instead.
//  * The AMR compact-object tracker uses type = BH for the black hole and
//    type = NS for the neutron star (see the <z4c> block in the .athinput).
//  * BH excision is enabled in the <coord> block.
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  // Atmosphere (floor) values applied inside the BH region where Elliptica
  // returns unphysical or zero density.  Read from the same floor parameters
  // used by the MHD solver so the atmosphere is consistent.
  Real dfloor = pin->GetOrAddReal("mhd", "dfloor", 1.28e-21);
  Real pfloor = pin->GetOrAddReal("mhd", "tfloor", 1.58255e-19);

  // Initialize the Elliptica data reader
  Elliptica_ID_Reader_T *idr =
    elliptica_id_reader_init(fname.c_str(), "generic");

  // For BHNS there is only one neutron star, so we supply a single EoS table
  // path (key "NS_EoS_table_path") rather than the two-star keys used for BNS.
  if (tab_path != "__na__") {
  ...
  for (int m = 0; m < nmb; m++) {
    for (int k = 0; k < ncells3; k++) {
      for (int j = 0; j < ncells2; j++) {
        for (int i = 0; i < ncells1; i++) {
          // Extract metric quantities (valid everywhere)
          ...
          // Outside the neutron star (i.e. inside or near the BH) Elliptica
          // returns density ≤ 0.  Apply atmosphere floor values there so that
          // the conserved variables are well-defined everywhere from the start.
          Real rho = idr->field[i_rho][idx];
          Real p   = idr->field[i_p][idx];

          if (rho <= dfloor) {
            // BH region or vacuum: use atmosphere values and zero velocity
            host_w0(m, IDN, k, j, i) = dfloor;
            host_w0(m, IPR, k, j, i) = pfloor;
            host_w0(m, IVX, k, j, i) = 0.0;
            host_w0(m, IVY, k, j, i) = 0.0;
            host_w0(m, IVZ, k, j, i) = 0.0;
          } else {
            host_w0(m, IDN, k, j, i) = rho;
            host_w0(m, IPR, k, j, i) = (p > pfloor) ? p : pfloor;
            Real vu[3]               = {
...
// History function: tracks maximum rest-mass density (NS) and minimum lapse.
// The minimum lapse is useful for monitoring both the BH (where it is ~0) and
// the NS collapse.
void EllipticaBHNSHistory(HistoryData *pdata, Mesh *pm) {
  ...
  Kokkos::parallel_reduce(
    "BHNSHistSums", Kokkos::RangePolicy<>(DevExeSpace(), 0, nmkji),
